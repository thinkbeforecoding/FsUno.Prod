...
let ``The next player cannot play``() =
    ...
    |> When ( PlayCard { GameId = gameId; Player = 2; Card = green Eight } )
    |> Expect [ PlayerPlayedAtWrongTurn { GameId = gameId; Player = 2; Card = green Eight } ]
...
let ``The second next player can play``() =
    Given [ GameStarted { GameId = gameId; PlayerCount = 4; FirstCard = red Three; FirstPlayer = 0 } 
            CardPlayed { GameId = gameId; Player = 0; Card = green Three; NextPlayer = 1}
            CardPlayed { GameId = gameId; Player = 1; Card = Skip Green; NextPlayer = 3} ]
    |> When ( PlayCard { GameId = gameId; Player = 3; Card = green Eight } )
    |> Expect [ CardPlayed { GameId = gameId; Player = 3; Card = green Eight; NextPlayer = 0 } ]

[<Fact>]
let ``skip should skip when counterclockwise``() =
    Given [ GameStarted { GameId = gameId; PlayerCount = 4; FirstCard = red Three; FirstPlayer = 0 } 
            CardPlayed { GameId = gameId; Player = 0; Card = green Three; NextPlayer = 1}
            CardPlayed { GameId = gameId; Player = 1; Card = green Eight; NextPlayer = 2}
