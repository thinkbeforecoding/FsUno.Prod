...
With several Aggregates we'll have to add Process and Proccess Managers to orchestrate the
reactions to events, and ensure the global consistency of the system.

We'll also have to manage projections over serveral aggregates. And make the infrastructure code
a bit more generic to not wire it directly to the only aggregate.

But before this happen: Desing !
...
process for something awsome !

## Algebra

The first question was whether to change the ``empty`` member and make it a static member, same
thing for the ``apply`` function.
...
Most of mathematatical structures are defined using sets (here S), functions (here +) and specific set items
(here n).
...
A quadruple (State, handle, apply, empty) with the rules and signature you know. ``handle`` and ``apply``
...
you can hover replay definition to see type constraints:
*)
...
it can be used on any type that has ``initial`` and apply members that have expected signature.
And the inference still works by return type, so any hint to the compiler indicating
that a specific Aggregate state type is in use, there is no need for further type annotation.
*)
    let state : State =
...
a high order function function to use it.

We can see how we'll implement more generic command handlers then.
